import { updateLanguageSelection } from '../common/internal_api/api_methods.ts';
import { effect, signal, Signal } from 'https://esm.sh/v135/@preact/signals-core@1.5.1/denonext/signals-core.mjs';
...
export function LanguageSelection (props: {
	...
	const updateTranslationLang = effect(() =>
			translationLang.value !== translationLangInit ?
				updateLanguageSelection({type: 'translation', value: translationLang.value})
					.then(res => window.location.reload()) :
				undefined
	);
	...
	return (
		<div className="flex flex-col">
			<div className={"mr-5"}>
				...
				>
					{ availableLanguages.map(lang => <option value={lang}>{lang}</option>) }
				</select>
			</div>
			<div>
